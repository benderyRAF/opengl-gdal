...
shape::shape() { points = vector<vector<glm::vec2>>(); }
...
bool shape::load(string filename) {
  ...
  // Taking from
  // http://www.compsci.wm.edu/SciClone/documentation/software/geo/gdal-1.9.0/html/ogr/ogr_apitut.html
  ...
  while ((poFeature = layer->GetNextFeature()) != NULL) {
    ...
    if (poGeometry != NULL &&
      ...
      OGRLineString* ls = (OGRLineString*)poGeometry->toLineString();
      points.push_back(vector<glm::vec2>());
...
bool shape::createMesh(OGRSpatialReference* sr2, glm::vec2 origin,
                       glm::vec2 scale, terrain& t) {
  ...
  if (!Renderer.addShader(
          GL_VERTEX_SHADER,
          AssetManager::GetAppPath() + "../../../../code/shader/linevert.shader")) {
    cout << "Failed to add shader" << endl;
    return false;
  }
  if (!Renderer.addShader(
          GL_FRAGMENT_SHADER,
          AssetManager::GetAppPath() + "../../../../code/shader/linefrag.shader")) {
    cout << "Failed to add shader" << endl;
    return false;
  }
  ...
  for (int i = 0; i < points.size(); i++) {
    ...
    for (int j = 0; j < points[i].size(); j++) {
      ...
      Vertex temp = {{(float)x, (float)t.SampleTerrain(orig), (float)y},
                     {(float)1, (float)0, (float)0},
                     {(float)1, (float)1}};
      // cout << "Vertex: " << temp.position.x << " " << temp.position.y << " "
      // << temp.position.z << endl;
      if (first) {
        ...
        if (j == points[i].size() - 1) {
          ...
          temp.position.z += 5;
          temp.position.y =
              t.SampleTerrain2(glm::vec2(temp.position.x, temp.position.z));
          vertexs.push_back(temp);
          temp.position.y += 40;
      ...
      if (j + 1 < points[i].size()) {
        ...
        transform->Transform(1, &x, &y);
        // cout << "After" << x << " " << y << " " << origin.x << " " <<
        // origin.y<< endl;
        orig = glm::vec2(x, y);
        x = x - origin.x;
        y = origin.y - y;

        Vertex temp2 = {{(float)x, (float)t.SampleTerrain(orig), (float)y},
                        {(float)1, (float)0, (float)0},
                        {1, 1}};
        // cout << "Vertex: " << temp2.position.x << " " << temp2.position.y <<
        // " " << temp2.position.z << endl; cout << "ORIGIN: " << origin.x << " "
        // << origin.y << " " << temp2.position.z << endl;
  ...
  elements.bindBuffer();
  if (veccount != 0) {
    elements.allocateBufferData(sizeof(int) * indicies.size(), &indicies[0],
                                GL_STATIC_DRAW);
  }
  return true;
}
void shape::render(glm::mat4& view, glm::mat4& projection) {
  ...
  Renderer.setGLVertexAttribPointer("v_position", 3, GL_FLOAT, GL_FALSE,
                                    sizeof(Vertex), 0);
  // Renderer.setGLVertexAttribPointer("v_color",3, GL_FLOAT,
